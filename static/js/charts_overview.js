...
function time_series(divId, data, year) {
  ...
  function makeChart() {
    ...
    let yAxisRight = d3.axisRight().scale(scaleCoverage).ticks(5);
    ...
    let coverageLineMissing = tooltip.append("path").attr("d", lineCoverage(data)).attr("class", "line dashed").style("stroke", "red");

    let coverageLine = tooltip.append("path").attr("d", lineCoverage(filteredDataCoverage)).attr("class", "line").style("stroke", colorCoverage);
    ...
    // Circles.
    if (coverage != "null") {
      let coverageCircle = tooltip.append("circle").attr("cx", scaleTime(year)).attr("cy", scaleCoverage(coverage)).attr("r", circleRadius).style("fill", colorCoverage);
    }
